...
module.exports.Dashing = function Dashing() {
  ...
  // Load route files
  app.use(router);
  // var route_path = [dashing.root, 'routes'].join(path.sep);
  // console.log("route_path", route_path);
  // logger.info('Searching for routes in', route_path);
  // fs.readdir(route_path, function(err, files) {
  //   console.log("files", files);
  //   if (err) throw err;
  //   for (var i in files) {
  //     console.log("files[i]", files[i]);
  //     var file = [route_path, files[i]].join(path.sep);
  //     fileMathced = file.match(/(\w*)\.route.js$/);
  //     console.log("regex", fileMathced);

  //     if (fileMathced) {
  //       logger.info('Loading route file:', files[i]);
  //       try {
  //           console.log("use routes files", fileMathced.input);
  //           var routeMiddleware = require(fileMathced.input);
  //           console.log("app", app);
